...
const questionList = computed(() => {
  ...
  return rawList.value.map(item => ({
    ...
    time: item.question.date,
    status: item.question.isSolved ? "已回复" : "未解决",
  }))
})
...
const getQuestionList = async (page, pageSize, queryName, type, isChecked = 1) => {
  // 默认参数说明：isChecked -> 默认只显示已通过审核的问题
  try {
    let data = (await userQuestionListService(page, pageSize, queryName, type, isChecked)).data
    // console.log(data);
    // if (data.pages === 0) {
    //   currentPage.value = 1
    ...
    //   await router.push({path, query})
    //   data = (await userQuestionListService(currentPage.value, pageSize, queryName, type, isChecked)).data
    // }
    // 去除原来的逻辑
    totalPages.value = data.pages
...
function onClickTab(item) {
  ...
  getQuestionList(currentPage.value, pageSize.value, query.queryName, query.type)
}
...
<template>
  <div class="flex flex-col flex-1">
    ...
    <Card v-for="question in questionList" :info="question" type="需求" class="m-3 mb-2"  />
    <div class="grow" />
