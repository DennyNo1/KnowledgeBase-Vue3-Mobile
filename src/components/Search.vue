...
<template>
  <div>
    <!--  使用div盒子包裹消除报错  -->
    <img
      src="../assets/China_Telecom_logo.svg"
      class="w-full p-8 my-4 max-w-sm mx-auto"
      alt=""
    />
    <div class="relative w-11/12 mx-auto">
      ...
      <div
        v-if="showDropdown"
        class="absolute left-0 mt-2 backdrop-blur-xl bg-white/50 border border-gray-300 rounded-lg shadow-lg overflow-hidden text-gray-500 z-10"
      >
        <div
          v-for="option in options"
          :key="option"
          class="px-4 py-2 cursor-pointer hover:bg-gray-300/50"
      ...
      <div class="m-4 mt-6">
        <van-radio-group
          ...
          class="flex flex-row justify-evenly justify-items-center mx-auto"
        >
          <van-radio class="mr-2" shape="dot" name="/article-list"
            >搜索课件</van-radio
          >
          <van-radio class="ml-2" shape="dot" name="/question-list"
            >搜索需求</van-radio
          >
        </van-radio-group>
      </div>
      <div
        class="flex flex-wrap mx-4 mt-8 justify-evenly justify-items-stretch gap-8"
      >
        <van-button class="custom-btn" to="/article-list">看课件</van-button>
